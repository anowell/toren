//! Caddy reverse proxy backend.
//!
//! Manages Caddy routes via the admin API for per-subdomain routing.
//! Each unique listen port gets its own Caddy server (named `station-{port}`),
//! with routes differentiated by hostname.

use anyhow::{Context, Result};
use tracing::{debug, info, warn};

use crate::RouteInfo;

/// Caddy-based proxy backend that manages routes via the Caddy admin API.
pub struct CaddyBackend {
    client: reqwest::Client,
    admin_url: String,
}

impl CaddyBackend {
    /// Create a new CaddyBackend targeting the given admin API URL.
    pub fn new(admin_url: String) -> Self {
        Self {
            client: reqwest::Client::new(),
            admin_url,
        }
    }

    /// Check that the Caddy admin API is reachable. Logs a warning if not.
    pub async fn check_health(&self) -> Result<()> {
        let url = format!("{}/config/", self.admin_url);
        match self.client.get(&url).send().await {
            Ok(r) if r.status().is_success() => {
                info!("Caddy admin API reachable");
            }
            Ok(r) => {
                warn!(
                    "Caddy admin API returned status {}: proxy routes may not work",
                    r.status()
                );
            }
            Err(e) => {
                warn!(
                    "Caddy admin API not reachable ({}): proxy routes will not work",
                    e
                );
            }
        }
        Ok(())
    }

    /// Add a proxy route (upsert). Removes any existing route for the same
    /// host+port first, creates the per-port server if needed, then adds the route.
    pub async fn add_route(
        &self,
        host: &str,
        upstream: &str,
        port: u16,
        tls: bool,
    ) -> Result<()> {
        // Remove existing route for this host+port to avoid duplicates
        self.remove_route(host, port).await?;

        // Ensure the per-port server exists
        self.ensure_port_server(port, tls).await?;

        let server_name = server_name_for_port(port);
        let route_id = route_id(host, port);
        let route = build_route(&route_id, host, upstream);

        let url = format!(
            "{}/config/apps/http/servers/{}/routes",
            self.admin_url, server_name
        );
        let resp = self
            .client
            .post(&url)
            .json(&route)
            .send()
            .await
            .context("Failed to add Caddy route")?;

        if resp.status().is_success() {
            info!(
                "Added route: {} -> {} on :{} (@{})",
                host,
                normalize_upstream(upstream),
                port,
                route_id
            );
            Ok(())
        } else {
            let body = resp.text().await.unwrap_or_default();
            anyhow::bail!("Failed to add route for {} on :{}: {}", host, port, body)
        }
    }

    /// Remove a route by host and port.
    pub async fn remove_route(&self, host: &str, port: u16) -> Result<()> {
        let route_id = route_id(host, port);
        let url = format!("{}/id/{}", self.admin_url, route_id);
        let resp = self
            .client
            .delete(&url)
            .send()
            .await
            .context("Failed to remove Caddy route")?;

        if resp.status().is_success() {
            info!("Removed route @{}", route_id);
            Ok(())
        } else {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            if status.as_u16() == 404 {
                debug!("Route @{} not found (already removed?)", route_id);
                Ok(())
            } else {
                anyhow::bail!("Failed to remove route @{}: {}", route_id, body)
            }
        }
    }

    /// Remove all routes matching a given host across all station-managed servers.
    pub async fn remove_routes_for_host(&self, host: &str) -> Result<()> {
        let routes = self.list_routes().await?;
        let matching: Vec<_> = routes.iter().filter(|r| r.host == host).collect();

        if matching.is_empty() {
            debug!("No routes found for host {}", host);
            return Ok(());
        }

        for route in matching {
            self.remove_route(&route.host, route.port).await?;
        }

        Ok(())
    }

    /// List all active station-managed proxy routes.
    pub async fn list_routes(&self) -> Result<Vec<RouteInfo>> {
        let url = format!("{}/config/apps/http/servers/", self.admin_url);
        let resp = self
            .client
            .get(&url)
            .send()
            .await
            .context("Failed to connect to Caddy admin API")?;

        if !resp.status().is_success() {
            return Ok(vec![]);
        }

        let servers: serde_json::Value = resp
            .json()
            .await
            .context("Failed to parse Caddy servers response")?;

        let mut routes = Vec::new();

        if let Some(obj) = servers.as_object() {
            for (server_name, server_config) in obj {
                if !server_name.starts_with("station-") {
                    continue;
                }

                let port = server_name
                    .strip_prefix("station-")
                    .and_then(|p| p.parse::<u16>().ok())
                    .unwrap_or(0);

                let tls = server_config.get("tls_connection_policies").is_some();

                if let Some(route_array) = server_config["routes"].as_array() {
                    for route in route_array {
                        let host = route["match"][0]["host"][0]
                            .as_str()
                            .unwrap_or("?")
                            .to_string();
                        let upstream = route["handle"][0]["upstreams"][0]["dial"]
                            .as_str()
                            .unwrap_or("?")
                            .to_string();

                        routes.push(RouteInfo {
                            host,
                            upstream,
                            port,
                            tls,
                        });
                    }
                }
            }
        }

        Ok(routes)
    }

    /// Ensure a per-port Caddy server exists with a routes array.
    /// Creates the server if missing. If it exists but `routes` was removed
    /// (e.g. after all routes were deleted), re-creates the empty array.
    async fn ensure_port_server(&self, port: u16, tls: bool) -> Result<()> {
        let server_name = server_name_for_port(port);
        let server_url = format!(
            "{}/config/apps/http/servers/{}",
            self.admin_url, server_name
        );

        // Check if server already exists (Caddy returns null/200 for missing keys,
        // so we must verify the body is an actual JSON object)
        if let Ok(r) = self.client.get(&server_url).send().await {
            if r.status().is_success() {
                let body: serde_json::Value =
                    r.json().await.unwrap_or(serde_json::Value::Null);
                if body.is_object() {
                    // Server exists — verify routes array exists
                    if body.get("routes").and_then(|v| v.as_array()).is_some() {
                        debug!("Caddy server '{}' ready", server_name);
                        return Ok(());
                    }
                    // Routes array missing — recreate it
                    debug!(
                        "Caddy server '{}' missing routes array, recreating",
                        server_name
                    );
                    let routes_url = format!("{}/routes", server_url);
                    let resp = self
                        .client
                        .put(&routes_url)
                        .json(&serde_json::json!([]))
                        .send()
                        .await
                        .context("Failed to connect to Caddy admin API")?;
                    if resp.status().is_success() {
                        return Ok(());
                    }
                    let body = resp.text().await.unwrap_or_default();
                    anyhow::bail!(
                        "Failed to recreate routes array for Caddy server '{}': {}",
                        server_name,
                        body
                    );
                }
                // Body is null — server doesn't actually exist, fall through to create
            }
        }

        // Create server config for this port
        let listen = format!(":{}", port);
        let mut server_config = serde_json::json!({
            "listen": [listen],
            "routes": []
        });

        if tls {
            server_config["tls_connection_policies"] = serde_json::json!([{}]);
        } else {
            server_config["automatic_https"] = serde_json::json!({"disable": true});
        }

        let resp = self
            .client
            .put(&server_url)
            .json(&server_config)
            .send()
            .await
            .context("Failed to connect to Caddy admin API")?;

        if resp.status().is_success() {
            info!(
                "Created Caddy server '{}' listening on :{}",
                server_name, port
            );
            Ok(())
        } else {
            let body = resp.text().await.unwrap_or_default();
            anyhow::bail!("Failed to create Caddy server '{}': {}", server_name, body)
        }
    }
}

/// Generate a Caddy server name from a port number.
fn server_name_for_port(port: u16) -> String {
    format!("station-{}", port)
}

/// Generate a deterministic route ID from a hostname and port.
fn route_id(host: &str, port: u16) -> String {
    format!("station-{}-{}", host.replace('.', "-"), port)
}

/// Normalize an upstream value to `host:port` format for Caddy's `dial` field.
/// Handles: bare port ("8001"), host:port ("localhost:8001"),
/// full URL ("http://localhost:8001").
/// Paths are stripped with a warning since Caddy's dial only accepts host:port.
fn normalize_upstream(upstream: &str) -> String {
    let stripped = upstream
        .trim_start_matches("http://")
        .trim_start_matches("https://");

    if stripped.parse::<u16>().is_ok() {
        return format!("localhost:{}", stripped);
    }

    // Strip path component — Caddy dial only supports host:port
    if let Some(slash_pos) = stripped.find('/') {
        let host_port = &stripped[..slash_pos];
        let path = &stripped[slash_pos..];
        tracing::warn!(
            "Stripping path '{}' from upstream '{}' — Caddy dial only supports host:port",
            path,
            upstream
        );
        return host_port.to_string();
    }

    stripped.to_string()
}

/// Build a Caddy route JSON object.
fn build_route(route_id: &str, host: &str, upstream: &str) -> serde_json::Value {
    serde_json::json!({
        "@id": route_id,
        "match": [{
            "host": [host]
        }],
        "handle": [{
            "handler": "reverse_proxy",
            "upstreams": [{
                "dial": normalize_upstream(upstream)
            }]
        }]
    })
}

#[cfg(test)]
mod tests {
    use super::*;
    use wiremock::matchers::{method, path};
    use wiremock::{Mock, MockServer, ResponseTemplate};

    fn test_backend(admin_url: &str) -> CaddyBackend {
        CaddyBackend::new(admin_url.to_string())
    }

    // -- ensure_port_server tests --

    #[tokio::test]
    async fn test_ensure_port_server_creates_new_server() {
        let mock = MockServer::start().await;
        let backend = test_backend(&mock.uri());

        Mock::given(method("GET"))
            .and(path("/config/apps/http/servers/station-8080"))
            .respond_with(ResponseTemplate::new(404))
            .expect(1)
            .mount(&mock)
            .await;

        Mock::given(method("PUT"))
            .and(path("/config/apps/http/servers/station-8080"))
            .respond_with(ResponseTemplate::new(200))
            .expect(1)
            .mount(&mock)
            .await;

        backend.ensure_port_server(8080, false).await.unwrap();

        let requests = mock.received_requests().await.unwrap();
        let put_req = requests
            .iter()
            .find(|r| r.method == reqwest::Method::PUT)
            .unwrap();
        let body: serde_json::Value = serde_json::from_slice(&put_req.body).unwrap();

        assert_eq!(body["listen"], serde_json::json!([":8080"]));
        assert_eq!(body["routes"], serde_json::json!([]));
        assert_eq!(body["automatic_https"]["disable"], serde_json::json!(true));
        assert!(body.get("tls_connection_policies").is_none());
    }

    #[tokio::test]
    async fn test_ensure_port_server_creates_tls_server() {
        let mock = MockServer::start().await;
        let backend = test_backend(&mock.uri());

        Mock::given(method("GET"))
            .and(path("/config/apps/http/servers/station-443"))
            .respond_with(ResponseTemplate::new(404))
            .expect(1)
            .mount(&mock)
            .await;

        Mock::given(method("PUT"))
            .and(path("/config/apps/http/servers/station-443"))
            .respond_with(ResponseTemplate::new(200))
            .expect(1)
            .mount(&mock)
            .await;

        backend.ensure_port_server(443, true).await.unwrap();

        let requests = mock.received_requests().await.unwrap();
        let put_req = requests
            .iter()
            .find(|r| r.method == reqwest::Method::PUT)
            .unwrap();
        let body: serde_json::Value = serde_json::from_slice(&put_req.body).unwrap();

        assert_eq!(body["listen"], serde_json::json!([":443"]));
        assert_eq!(body["tls_connection_policies"], serde_json::json!([{}]));
        assert!(body.get("automatic_https").is_none());
    }

    #[tokio::test]
    async fn test_ensure_port_server_null_200_creates_server() {
        let mock = MockServer::start().await;
        let backend = test_backend(&mock.uri());

        Mock::given(method("GET"))
            .and(path("/config/apps/http/servers/station-8080"))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::Value::Null))
            .expect(1)
            .mount(&mock)
            .await;

        Mock::given(method("PUT"))
            .and(path("/config/apps/http/servers/station-8080"))
            .respond_with(ResponseTemplate::new(200))
            .expect(1)
            .mount(&mock)
            .await;

        backend.ensure_port_server(8080, false).await.unwrap();
    }

    #[tokio::test]
    async fn test_ensure_port_server_existing_with_routes() {
        let mock = MockServer::start().await;
        let backend = test_backend(&mock.uri());

        let server_body = serde_json::json!({
            "listen": [":8080"],
            "routes": [{"@id": "some-route"}]
        });
        Mock::given(method("GET"))
            .and(path("/config/apps/http/servers/station-8080"))
            .respond_with(ResponseTemplate::new(200).set_body_json(&server_body))
            .expect(1)
            .mount(&mock)
            .await;

        Mock::given(method("PUT"))
            .and(path("/config/apps/http/servers/station-8080"))
            .respond_with(ResponseTemplate::new(200))
            .expect(0)
            .mount(&mock)
            .await;

        backend.ensure_port_server(8080, false).await.unwrap();
    }

    #[tokio::test]
    async fn test_ensure_port_server_repairs_missing_routes() {
        let mock = MockServer::start().await;
        let backend = test_backend(&mock.uri());

        let server_body = serde_json::json!({
            "listen": [":8080"]
        });
        Mock::given(method("GET"))
            .and(path("/config/apps/http/servers/station-8080"))
            .respond_with(ResponseTemplate::new(200).set_body_json(&server_body))
            .expect(1)
            .mount(&mock)
            .await;

        Mock::given(method("PUT"))
            .and(path("/config/apps/http/servers/station-8080/routes"))
            .respond_with(ResponseTemplate::new(200))
            .expect(1)
            .mount(&mock)
            .await;

        backend.ensure_port_server(8080, false).await.unwrap();

        let requests = mock.received_requests().await.unwrap();
        let put_req = requests
            .iter()
            .find(|r| r.method == reqwest::Method::PUT)
            .unwrap();
        let body: serde_json::Value = serde_json::from_slice(&put_req.body).unwrap();
        assert_eq!(body, serde_json::json!([]));
    }

    // -- add_route / remove_route tests --

    #[tokio::test]
    async fn test_add_route_removes_existing_first() {
        let mock = MockServer::start().await;
        let backend = test_backend(&mock.uri());
        let host = "app.toren.lvh.me";
        let rid = route_id(host, 8080);

        Mock::given(method("DELETE"))
            .and(path(format!("/id/{}", rid)))
            .respond_with(ResponseTemplate::new(200))
            .expect(1)
            .mount(&mock)
            .await;

        Mock::given(method("GET"))
            .and(path("/config/apps/http/servers/station-8080"))
            .respond_with(
                ResponseTemplate::new(200).set_body_json(serde_json::json!({
                    "listen": [":8080"],
                    "routes": []
                })),
            )
            .expect(1)
            .mount(&mock)
            .await;

        Mock::given(method("POST"))
            .and(path("/config/apps/http/servers/station-8080/routes"))
            .respond_with(ResponseTemplate::new(200))
            .expect(1)
            .mount(&mock)
            .await;

        backend
            .add_route(host, "localhost:3000", 8080, false)
            .await
            .unwrap();

        let requests = mock.received_requests().await.unwrap();
        let delete_idx = requests
            .iter()
            .position(|r| r.method == reqwest::Method::DELETE)
            .unwrap();
        let post_idx = requests
            .iter()
            .position(|r| r.method == reqwest::Method::POST)
            .unwrap();
        assert!(delete_idx < post_idx, "DELETE should happen before POST");
    }

    #[tokio::test]
    async fn test_remove_route_404_is_ok() {
        let mock = MockServer::start().await;
        let backend = test_backend(&mock.uri());

        let rid = route_id("app.toren.lvh.me", 8080);

        Mock::given(method("DELETE"))
            .and(path(format!("/id/{}", rid)))
            .respond_with(ResponseTemplate::new(404))
            .expect(1)
            .mount(&mock)
            .await;

        backend
            .remove_route("app.toren.lvh.me", 8080)
            .await
            .unwrap();
    }

    // -- list_routes tests --

    #[tokio::test]
    async fn test_list_routes_parses_servers() {
        let mock = MockServer::start().await;
        let backend = test_backend(&mock.uri());

        let servers = serde_json::json!({
            "station-443": {
                "listen": [":443"],
                "tls_connection_policies": [{}],
                "routes": [{
                    "match": [{"host": ["app.lvh.me"]}],
                    "handle": [{"handler": "reverse_proxy", "upstreams": [{"dial": "localhost:3000"}]}]
                }]
            },
            "station-8080": {
                "listen": [":8080"],
                "routes": [{
                    "match": [{"host": ["api.lvh.me"]}],
                    "handle": [{"handler": "reverse_proxy", "upstreams": [{"dial": "localhost:4000"}]}]
                }]
            },
            "other-server": {
                "listen": [":9999"],
                "routes": [{
                    "match": [{"host": ["external.example.com"]}],
                    "handle": [{"handler": "reverse_proxy", "upstreams": [{"dial": "localhost:5000"}]}]
                }]
            }
        });

        Mock::given(method("GET"))
            .and(path("/config/apps/http/servers/"))
            .respond_with(ResponseTemplate::new(200).set_body_json(&servers))
            .expect(1)
            .mount(&mock)
            .await;

        let routes = backend.list_routes().await.unwrap();

        // Should only include station-* servers
        assert_eq!(routes.len(), 2);

        let app_route = routes.iter().find(|r| r.host == "app.lvh.me").unwrap();
        assert_eq!(app_route.upstream, "localhost:3000");
        assert_eq!(app_route.port, 443);
        assert!(app_route.tls);

        let api_route = routes.iter().find(|r| r.host == "api.lvh.me").unwrap();
        assert_eq!(api_route.upstream, "localhost:4000");
        assert_eq!(api_route.port, 8080);
        assert!(!api_route.tls);
    }

    // -- unit tests --

    #[test]
    fn test_normalize_upstream_bare_port() {
        assert_eq!(normalize_upstream("8001"), "localhost:8001");
        assert_eq!(normalize_upstream("443"), "localhost:443");
    }

    #[test]
    fn test_normalize_upstream_host_port() {
        assert_eq!(normalize_upstream("localhost:5173"), "localhost:5173");
        assert_eq!(normalize_upstream("myhost:8080"), "myhost:8080");
    }

    #[test]
    fn test_normalize_upstream_url() {
        assert_eq!(normalize_upstream("http://localhost:5173"), "localhost:5173");
        assert_eq!(
            normalize_upstream("https://localhost:8443"),
            "localhost:8443"
        );
    }

    #[test]
    fn test_normalize_upstream_strips_path() {
        assert_eq!(
            normalize_upstream("http://localhost:8080/api"),
            "localhost:8080"
        );
        assert_eq!(
            normalize_upstream("http://localhost:8080/api/v2"),
            "localhost:8080"
        );
        assert_eq!(normalize_upstream("localhost:3000/path"), "localhost:3000");
    }

    #[test]
    fn test_server_name_for_port() {
        assert_eq!(server_name_for_port(443), "station-443");
        assert_eq!(server_name_for_port(8080), "station-8080");
    }

    #[test]
    fn test_route_id() {
        assert_eq!(
            route_id("one.toren.lvh.me", 443),
            "station-one-toren-lvh-me-443"
        );
        assert_eq!(
            route_id("one.toren.lvh.me", 80),
            "station-one-toren-lvh-me-80"
        );
    }

    #[test]
    fn test_build_route_normalizes_upstream() {
        let route = build_route("test-id", "one.lvh.me", "8001");
        let dial = route["handle"][0]["upstreams"][0]["dial"]
            .as_str()
            .unwrap();
        assert_eq!(dial, "localhost:8001");
    }
}
